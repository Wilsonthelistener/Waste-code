...
class Encoder(nn.Module):
  ...
  def __call__(self, x, *, train):
    ...
    """
    assert x.ndim == 3  # (batch, seq_len, emb)
...
dummy_input = jnp.ones((1, 8, 8, 8, 3))  # (batch, depth, height, width, channels)
rng = jax.random.PRNGKey(0)
params = model.init(rng, dummy_input, train=False)

def count_params(param_dict):
    return sum(jnp.prod(jnp.array(p.shape)) for p in jax.tree_util.tree_leaves(param_dict))
